...
| **Skills (canonical, in-repo)** | **Decided 2026-08-16**: `skills/` at the repo root is the canonical agent-skill set, bundled into the CLI at build time (no network fetch). Two vibekit-authored skills (`use-vibekit` — tool access paths incl. meta-tool harnesses and the shell fallback, account/keystore model, networks, signing, denominations; `vibekit-project-setup` — new/init/localnet/doctor) plus the upstream language/stack skills vendored from algorand-devrel/algorand-agent-skills (MIT) and maintained here; upstream's `algorand-project-setup` (AlgoKit CLI reference) dropped — it was actively misdirecting agents to algokit commands. AGENTS.md carries an algokit→vibekit command-precedence table for residual references |
