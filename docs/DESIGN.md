...
| **`vibekit explore`** (TUI) | Agent-native Lora: `@initlabs/vibekit-agent` running **in-process**, importing tool packages directly — no MCP hop, no hosted dependency, works offline against localnet. Ink component TUI: result cards from data + display hints, never from model prose | BYO API key / local model (Ollama, OpenAI-compatible) via CLI config file | Read-only by default; connect a keystore-daemon account and write tools unlock, every write behind an in-TUI approval card (the terminal's WalletConnect equivalent — no pairing state) |
