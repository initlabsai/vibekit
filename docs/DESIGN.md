...
- `packages/mcp/examples/` is the reference deployment (folded in from the
  former `apps/mcp`, 2026-08-23): a small set of files anyone can
...
| **Skills (canonical, in-repo)** | **Decided 2026-08-23**: `skills/` at the repo root remains the tool-agnostic canonical vendored set bundled into the CLI. Git-tracked relative symlinks expose the same tree through `.agents/skills` for Codex, `.claude/skills` for Claude, and `.grok/skills` for Grok. Four product skills replace the earlier per-library bundle: `use-vibekit` for operating inside configured projects, `build-on-algorand` for TypeScript Algorand development, `audit-algorand` for structured PuyaTs application and LogicSig security audits, and `build-on-vibekit` for extending the public VibeKit surface. `update-skill` records the maintenance workflow, primary-source map, provenance, and generated-surface gates for that set. `vibekit-project-setup` is folded into `use-vibekit` through progressive references. `build-on-algorand` vendors and refactors the useful TypeScript portions of `algorand-devrel/algorand-agent-skills` (MIT); it excludes Python and AlgoKit CLI workflows. The earlier upstream skills were removed after live runs showed weak models following their AlgoKit-heavy instructions over VibeKit's generated project guidance.                                             |
...
- **Phase 1 — Skeleton.** Fresh repo, tsconfig/turbo/changesets/CI,
  ...
  server library, reference deployment (now `packages/mcp/examples/`). ✅ **Done 2026-08-15**
...
- **Phase 6 — Orchestrator + TUI.** `@initlabs/vibekit-agent` (LLM
  ...
  **TUI dropped (2026-08-16, owner's call, same day it shipped):** live
  ...
  history keeps it). The `vibekit agent` stub was deleted 2026-08-23
  (owner call: not shipping it); Phase 8 claims its own entry point
  when it lands. **What survives,
