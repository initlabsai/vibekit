...
Status: **executing — Phases 0–5 complete, next: Phase 6 (orchestrator + TUI)** · Owner: Gabriel Kuettel · Last updated: 2026-08-15

> **Handover snapshot (2026-08-15).** Code: `~/Code/@initlabs/vibekit` = `github.com/initlabsai/vibekit` (private) — 12 packages + reference app + CLI, ~155 tests, all green (`bun install && bunx turbo run build typecheck test`). Live-verified: stateless MCP (2026-07-28 spec + legacy clients), 44 tools + 8 plugin tools, multi-network per-request selection, keystore-daemon signing, compose mode, app deploy — all E2E on localnet/testnet. **Phase 5 done**: `apps/cli` ships `vibekit init` (agents/skills/MCP config, ported from v1 and slimmed), `vibekit new` (template tarballs, live-verified), `vibekit localnet` (TS re-implementation of AlgoKit's sandbox.py — start/stop/reset/status/logs + kmd `fund`, live-verified E2E), and `vibekit mcp` (stdio via the `@initlabs/vibekit-mcp` library, 50 tools, keystore execute mode with compose fallback). The AlgoKit-free dev path is complete. **npm publish is staged but HELD for a 1.0 release** (gate between Phases 6 and 7). **Next: Phase 6 — `@initlabs/vibekit-agent` orchestrator + `vibekit explore` TUI.** Read §9 (products), §10 (state model — normative), §12 (migration plan) before structural changes. Hard rules in the repo's AGENTS.md.
