...
Returns the new application ID and app address.`,
    parameters: z.object({
      appSpec: z.string().optional().describe('The ARC-56 or ARC-32 app spec JSON as a string. For small specs only.'),
      appSpecPath: z.string().optional().describe('Path to ARC-56/ARC-32 app spec JSON file. Recommended for large specs (>2KB) to avoid truncation issues.'),
      method: z.string().optional().describe('ABI method name for create (e.g., "createApplication"). If not provided, uses bare create.'),
      args: z.array(z.any()).optional().describe('Arguments for the ABI create method, in order.'),
      deployTimeParams: z.record(z.any()).optional().describe('Optional template parameters for deploy-time substitution'),
      sender: z.string().optional().describe('Sender address. Must be an account in a KMD wallet (use list_accounts to see available). Defaults to the localnet dispenser if not specified.'),
    }),
  ...
  },
  {
    ...
    parameters: z.object({
      ...
      appSpec: z.string().optional().describe('The full ARC-56 or ARC-32 app spec JSON as a string. For small specs only.'),
      appSpecPath: z.string().optional().describe('Path to ARC-56/ARC-32 app spec JSON file. Recommended for large specs (>2KB) to avoid truncation issues.'),
      methodSignature: z.string().optional().describe('ARC-4 method signature (e.g., "hello(string)string"). Use for simple calls without the full app spec.'),
      method: z.string().optional().describe('Method name to call (required when using appSpec/appSpecPath). Ignored when using methodSignature.'),
      ...
      extraFee: z.number().optional().describe('Extra fee in microALGO to cover inner transactions'),
  ...
  },
  {
    ...
    parameters: z.object({
      appId: z.number().describe('The application ID'),
      appSpec: z.string().optional().describe('App spec JSON for ABI method call. For small specs only.'),
      appSpecPath: z.string().optional().describe('Path to ARC-56/ARC-32 app spec JSON file. Recommended for large specs (>2KB) to avoid truncation issues.'),
      method: z.string().optional().describe('ABI method name for opt-in (e.g., "opt_in", "optIn"). Required with appSpec/appSpecPath.'),
      args: z.array(z.any()).optional().describe('Method arguments if the opt-in method requires them'),
      sender: z.string().optional().describe('Sender address. Defaults to active account.'),
      extraFee: z.number().optional().describe('Extra fee in microALGO to cover inner transactions'),
      maxFee: z.number().optional().describe('Max fee in microALGO'),
    }),
  ...
  },
  {
    ...
    parameters: z.object({
      ...
      appSpec: z.string().optional().describe('App spec JSON for ABI method call. For small specs only.'),
      appSpecPath: z.string().optional().describe('Path to ARC-56/ARC-32 app spec JSON file. Recommended for large specs (>2KB) to avoid truncation issues.'),
      method: z.string().optional().describe('ABI method name for close-out (e.g., "close_out", "closeOut"). Required with appSpec/appSpecPath.'),
      args: z.array(z.any()).optional().describe('Method arguments if the close-out method requires them'),
      sender: z.string().optional().describe('Sender address. Defaults to active account.'),
      extraFee: z.number().optional().describe('Extra fee in microALGO to cover inner transactions'),
      maxFee: z.number().optional().describe('Max fee in microALGO'),
    }),
  ...
  },
  {
    ...
    parameters: z.object({
      ...
      appSpec: z.string().optional().describe('App spec JSON for ABI method call. For small specs only.'),
      appSpecPath: z.string().optional().describe('Path to ARC-56/ARC-32 app spec JSON file. Recommended for large specs (>2KB) to avoid truncation issues.'),
      method: z.string().optional().describe('ABI method name for deletion (e.g., "delete", "destroy"). Required with appSpec/appSpecPath.'),
      args: z.array(z.any()).optional().describe('Method arguments if the delete method requires them'),
      sender: z.string().optional().describe('Sender address. Defaults to active account.'),
      extraFee: z.number().optional().describe('Extra fee in microALGO to cover inner transactions'),
      maxFee: z.number().optional().describe('Max fee in microALGO'),
    }),
  ...
  },
  {
    ...
    handler: async ({ args, resolveAppSpec }) =>
      appListMethods(args, resolveAppSpec),
  },
]
