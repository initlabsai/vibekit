...
export const assetWriteTools: ToolDefinition[] = [
  {
    ...
    parameters: z.object({
      ...
      assetName: z
        .string()
        .optional()
        .describe('Full asset name (max 32 bytes). Example: "My Token"'),
      unitName: z.string().optional().describe('Ticker/unit name (max 8 bytes). Example: "MTK"'),
      url: z
        .string()
        .optional()
        .describe('Metadata URL (max 96 bytes). For NFTs, often points to IPFS metadata'),
      metadataHash: z
        .string()
        .optional()
        .describe('Optional 32-byte metadata hash (hex or base64 encoded)'),
      defaultFrozen: z
        .boolean()
        .optional()
        .describe('If true, asset holdings are frozen by default. Default: false'),
      manager: z
        .string()
        .optional()
        .describe(
      ...
      reserve: z
        .string()
        .optional()
        .describe('Address holding non-circulating supply (informational only)'),
    ...
    handler: async ({ algorand, args, resolveSender }) =>
      createAsset(algorand, args, resolveSender),
  },
  {
    name: 'asset_transfer',
    description:
    ...
    handler: async ({ algorand, args, resolveSender }) =>
      transferAsset(algorand, args, resolveSender),
  },
  {
    name: 'asset_opt_in',
    description:
  ...
  },
  {
    ...
    parameters: z.object({
      ...
      creator: z
        .string()
        .describe('Address to return remaining assets to (typically the asset creator)'),
      sender: z.string().optional().describe('Account to opt out. Defaults to active account'),
      ensureZeroBalance: z
        .boolean()
        .optional()
        .describe('If true, fail if account has non-zero balance. Default: true'),
    }),
    handler: async ({ algorand, args, resolveSender }) =>
      optOutAsset(algorand, args, resolveSender),
  },
  {
    name: 'asset_freeze',
    description:
    ...
    parameters: z.object({
      ...
      sender: z
        .string()
        .optional()
        .describe('Must be the asset freeze address. Defaults to active account'),
    }),
    handler: async ({ algorand, args, resolveSender }) =>
      freezeAsset(algorand, args, resolveSender),
  },
  {
    name: 'asset_config',
    description:
    ...
    parameters: z.object({
      ...
      manager: z
        .string()
        .optional()
        .describe('New manager address. Empty string removes manager permanently'),
      reserve: z.string().optional().describe('New reserve address. Empty string removes reserve'),
      freeze: z
        .string()
        .optional()
        .describe('New freeze address. Empty string removes freeze capability'),
      clawback: z
        .string()
        .optional()
        .describe('New clawback address. Empty string removes clawback capability'),
      sender: z.string().optional().describe('Must be current manager. Defaults to active account'),
    }),
    handler: async ({ algorand, args, resolveSender }) =>
      configAsset(algorand, args, resolveSender),
  },
  {
    name: 'asset_destroy',
    description:
    ...
    parameters: z.object({
      ...
      sender: z
        .string()
        .optional()
        .describe('Must be the asset manager. Defaults to active account'),
    }),
    handler: async ({ algorand, args, resolveSender }) =>
      destroyAsset(algorand, args, resolveSender),
  },
  {
    name: 'get_asset_info',
    description:
