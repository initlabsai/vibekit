...
function parse(teal: string): { instructions: Instruction[]; labels: Map<string, number>; version?: number } {
...
function byteConstants(inst: Instruction, pool: string[]): string[] {
  switch (inst.op) {
    ...
    case 'method':
      try {
        return [toHex(ABIMethod.fromSignature(inst.args.join(' ').replace(/^"|"$/g, '')).getSelector())]
      } catch {
        return []
      }
    default:
...
export function analyzeTeal(teal: string): TealAnalysis {
  ...
  for (const inst of instructions) {
    if (inst.op === 'bytecblock') bytePool.push(...inst.args.map((a) => a.replace(/^0x/, '').toLowerCase()))
  ...
  instructions.forEach((inst, index) => {
    ...
    if (inst.op === 'match' && isArg0(instructions[index - 1])) {
      ...
      for (let back = 2; back <= count + 1; back++) {
        for (const hex of byteConstants(instructions[index - back] ?? { op: '', args: [] }, bytePool)) {
