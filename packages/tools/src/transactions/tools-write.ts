...
const transactionSchema = z.object({
  ...
  closeRemainderTo: z
    .string()
    .optional()
    .describe('(payment) Address to receive remaining balance. Closes the account.'),
  // Asset fields
  ...
  metadataHash: z
    .string()
    .optional()
    .describe('(asset_create) 32-byte metadata hash (64 hex chars or 44 base64 chars)'),
  defaultFrozen: z
    .boolean()
    .optional()
    .describe('(asset_create) Whether holdings are frozen by default'),
  ...
  frozen: z
    .boolean()
    .optional()
    .describe('(asset_freeze) Whether to freeze (true) or unfreeze (false)'),
  clawbackTarget: z
    .string()
    .optional()
    .describe('(asset_transfer) For clawback: the account to claw back from'),
  closeAssetTo: z
    .string()
    .optional()
    .describe('(asset_opt_out, asset_transfer) Account to send remaining balance to'),
  ensureZeroBalance: z
    .boolean()
    .optional()
    .describe('(asset_opt_out) Fail if account has non-zero balance. Default: true'),
  // App call fields
  appId: z.number().optional().describe('(app_*) The application ID'),
  methodSignature: z
    .string()
    .optional()
    .describe('(app_call) ARC-4 method signature (e.g., "hello(string)string")'),
  ...
  extraFee: z
    .number()
    .optional()
    .describe('(app_call) Extra fee in microALGO to cover inner transactions'),
  ...
  {"type": "app_call", "appId": 456, "methodSignature": "withdraw()void", "maxFee": 5000}
    ...
    parameters: z.object({
      transactions: z
        ...
        .min(1)
        .max(16)
        .describe('Array of transactions to execute atomically (max 16)'),
      populateAppCallResources: z
        .boolean()
        .optional()
        .describe(
          'Auto-populate app call resources (accounts, apps, assets, boxes) via simulation. Defaults to true.'
        ),
      coverAppCallInnerTransactionFees: z
        .boolean()
        .optional()
        .describe(
...
{"transactions": [...], "execTraceConfig": {"enable": true, "stateChange": true}}`,
    parameters: z.object({
      transactions: z
        ...
        .min(1)
        .max(16)
        .describe('Array of transactions to simulate (max 16)'),
      allowMoreLogging: z.boolean().optional().describe('Lift log limits for verbose output'),
      allowUnnamedResources: z
        .boolean()
        .optional()
        .describe('Allow unnamed resources in app calls'),
      extraOpcodeBudget: z
        .number()
        .optional()
        .describe('Additional opcode budget for complex contracts'),
      execTraceConfig: z
        .object({
        ...
        .optional()
        .describe('Execution tracing configuration for debugging'),
    }),
    handler: async ({ algorand, args, resolveSender, resolveAppSpec }) => {
      const {
        transactions,
