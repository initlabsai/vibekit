...
const tool = (name: string) => [...contractTools, ...contractWriteTools].find((t) => t.name === name)!

describe('appSpecPath', () => {
  test('app_list_methods reads the spec from disk', async () => {
    const result = (await tool('app_list_methods').handler(fakeContext({}), { appSpecPath: SPEC_PATH })) as {
      name: string
  ...
  test('a missing file and a missing spec are distinct, clear errors', async () => {
    ...
    await expect(tool('app_deploy').handler(fakeContext({}), { sender: 'A' })).rejects.toMatchObject({
