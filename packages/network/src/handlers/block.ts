...
export async function lookupBlock(
  ...
  return {
    ...
    feesCollected:
      response.feesCollected != null ? Number(response.feesCollected) / MICROALGOS_PER_ALGO : undefined,
    proposerPayout:
      response.proposerPayout != null ? Number(response.proposerPayout) / MICROALGOS_PER_ALGO : undefined,
