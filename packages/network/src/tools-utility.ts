...
export const utilityTools: ToolDefinition[] = [
  ...
  },
  {
    ...
    parameters: z.object({
      numAssets: z.number().optional().describe('Number of assets opted into (includes created assets). Default: 0'),
      numCreatedApps: z.number().optional().describe('Number of apps created by this account. Default: 0'),
      numOptedInApps: z.number().optional().describe('Number of apps opted into (excluding created apps). Default: 0'),
      numExtraAppPages: z.number().optional().describe('Total extra app pages across all created apps. Default: 0'),
      numBoxes: z.number().optional().describe('Number of boxes across all apps created by this account. Default: 0'),
      totalBoxBytes: z.number().optional().describe('Sum of (nameLength + valueSize) for all boxes. Each box costs 2,500 + 400 * bytes. Default: 0'),
    }),
