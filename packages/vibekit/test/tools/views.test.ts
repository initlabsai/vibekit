...
// The ./tools/views subpath is the browser-safe import: its whole transitive
...
describe('views import graph', () => {
  test('src/tools/views.ts transitively imports only zod and schema modules', () => {
    const entry = resolve(import.meta.dir, '../../src/tools/views.ts')
    ...
    while (queue.length > 0) {
      ...
      const specifiers = [
        ...
        ...[...source.matchAll(/^(?:import|export)[\s\S]*?from\s+['"]([^'"]+)['"]/gm)].map(
          (m) => m[1]!,
        ),
