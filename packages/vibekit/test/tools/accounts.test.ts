...
describe('lookupAccount', () => {
  test('formats account with microALGO balances and bigint round fields', async () => {
    const ctx = fakeContext({
      indexer: {
        lookupAccountByID: () =>
          chainable({ account: fakeIndexerAccount(ADDR, BigInt(2_500_000)) }),
      },
    })
    ...
    const ctx = fakeContext({
      ...
      algod: {
        getAssetByID: () => {
          ...
          return chainable({
            index: 31566704n,
            params: { name: 'USDC', unitName: 'USDC', decimals: 6 },
          })
        },
      },
    })
...
describe('transactionQueryOf', () => {
  test('echoes only the filter keys that were set', () => {
    expect(transactionQueryOf({ txType: 'axfer', minRound: 5 })).toEqual({
      txType: 'axfer',
      minRound: 5,
    })
...
describe('never-funded address (indexer 404)', () => {
  const ctx = fakeContext({
    indexer: {
      lookupAccountByID: () =>
        failing(
          'Network request error. Received status 404 (Not Found): no accounts found for address: X',
        ),
    },
  })
                ...
                keyValue: [
                  {
                    ...
                    value: {
                      ...
                      uint: BigInt('18446744073709551615'),
                    },
                  },
                ],
              },
