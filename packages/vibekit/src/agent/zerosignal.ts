...
export function zeroSignalDaemonPath(
...
export function zeroSignalBaseUrl(
  ...
  try {
    ...
    const host =
      match[1] === '' || match[1] === '0.0.0.0' || match[1] === '::' ? '127.0.0.1' : match[1]
...
export function formatZeroSignalPrice(info: ZeroSignalModelInfo | undefined): string | undefined {
  ...
  if (info.inputUsdPer1M === undefined && info.outputUsdPer1M === undefined)
...
export async function listZeroSignalModels(
  ...
  if (!response.ok) {
    throw new Error(
      `ZeroSignal model catalog unavailable (${response.status}). ${zeroSignalSetupHint(baseUrl)}`,
    )
  }
