...
import { assetProfileSchema, type AssetProfile } from './schemas.js'

export { assetProfileSchema, type AssetProfile }
...
export function getPera(ctx: ToolContext): { get(path: string): Promise<unknown> } {
  ...
  if (!service) {
    throw new ToolError(
      ...
      'The pera plugin is not registered in this deployment',
    )
  }
  ...
  if (networkId !== 'mainnet' && networkId !== 'testnet') {
    throw new ToolError(
      'UNSUPPORTED_NETWORK',
      `Pera serves mainnet and testnet only, not ${networkId}`,
    )
  }
...
export function peraPlugin(): ToolPlugin {
  return {
    name: PLUGIN_NAME,
    description: 'Pera asset registry — verification tier, project identity (mainnet/testnet)',
    tools: peraTools,
    service: createPeraService(),
    views: { 'pera.asset': assetProfileSchema },
  }
}
