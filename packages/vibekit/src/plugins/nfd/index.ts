  ...
  return {
    clientFor(networkId) {
      ...
      if (networkId !== 'mainnet' && networkId !== 'testnet') {
        throw new ToolError(
          'UNSUPPORTED_NETWORK',
          `NFD is not available on ${networkId} (mainnet/testnet only)`,
        )
      }
...
export function getNfdClient(ctx: ToolContext): NfdApiClient {
  ...
  if (!service) {
    throw new ToolError(
      ...
      'The nfd plugin is not registered in this deployment',
    )
  }
...
async function nfdCall<T>(call: Promise<T>): Promise<T> {
  try {
    ...
    throw new ToolError(
      'NFD_ERROR',
      message ? `NFD: ${message}` : `NFD request failed (${name ?? 'unknown'})`,
    )
  }
}
...
export const nfdTools: AnyTool[] = [
  ...
  defineTool({
    ...
    handler: async (ctx, args) => {
      const result = await nfdCall(
        getNfdClient(ctx).reverseLookup([args.address], { view: 'full' }),
      )
  ...
  defineTool({
    ...
    handler: async (ctx, args) => {
      const result = await nfdCall(
        getNfdClient(ctx).reverseLookup(args.addresses, { view: 'thumbnail' }),
      )
      return {
