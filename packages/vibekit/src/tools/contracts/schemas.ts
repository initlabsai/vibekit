...
const stateValue = z.object({
  key: z
    .string()
    .describe('State key: app-spec name when resolvable, else UTF-8 decode of the raw key'),
  ...
  valueBase64: z
    .string()
    .optional()
    .describe('base64 of the exact value bytes (bytes-typed state only)'),
...
export const applicationProgramSchema = z.object({
  ...
  analysis: z.object({
    ...
    entrypoints: z
      .array(z.string())
      .describe(
        'Constants compared against ApplicationArgs 0: ARC-4 selectors as 0x-hex, string method names as text',
      ),
    selectors: z.array(z.string()).describe('The ARC-4 subset of entrypoints, bare hex'),
    arc4Returns: z
      .boolean()
      .describe('Logs return values behind the ARC-4 prefix 0x151f7c75 (sha512/256("return")[:4])'),
    strings: z.array(z.string()),
    stateKeys: z.object({
      global: z.array(z.string()),
      local: z.array(z.string()),
      box: z.array(z.string()),
    }),
    guards: z.object({ rekey: z.boolean(), closeRemainder: z.boolean(), assetClose: z.boolean() }),
    innerTransactions: z.number(),
    onCompletion: z.array(
      z.object({ action: onCompletionActionSchema, outcome: z.enum(['handled', 'rejected']) }),
    ),
  }),
  methods: z.array(
    z.object({
