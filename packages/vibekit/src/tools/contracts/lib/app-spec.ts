...
const arc32Schema = z.object({
  ...
  state: z
    .object({
      global: z.object({ num_uints: count.optional(), num_byte_slices: count.optional() }).optional(),
      local: z.object({ num_uints: count.optional(), num_byte_slices: count.optional() }).optional(),
    })
    .optional(),
  source: sourceSchema.optional(),
  byteCode: sourceSchema.optional(),
...
function scanTemplateVariables(source: { approval?: string; clear?: string } | undefined): string[] {
  if (!source) return []
...
export function normalizeAppSpec(appSpecJson: string): NormalizedAppSpec {
  ...
  if (format === 'arc56') {
    ...
    if (!parsed.success) throw invalid(`appSpec is not a valid ARC-56 file — ${zodDetail(parsed.error)}`)
  ...
  if (format === 'arc32') {
    ...
    if (!parsed.success) throw invalid(`appSpec is not a valid ARC-32 file — ${zodDetail(parsed.error)}`)
    const spec = parsed.data
    return {
      format,
  ...
  if (!parsed.success) throw invalid(`appSpec is not a valid ARC-4 contract — ${zodDetail(parsed.error)}`)
  return {
    format,
