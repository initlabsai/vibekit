...
export function estimateProgramTokens(bytes: number): { tokens: number; totalLines: number; pages: number } {
...
export async function getApplicationProgram(
  ...
  const bytecode = program === 'approval' ? app.params?.approvalProgram : app.params?.clearStateProgram
  if (!bytecode || bytecode.length === 0) {
    throw new ToolError('APP_NOT_FOUND', `Application ${args.applicationId} has no ${program} program`)
  }
  let teal: string
  try {
  ...
  const toLine = Math.min(lines.length, args.toLine ?? fromLine + PROGRAM_PAGE_LINES - 1, fromLine + MAX_PAGE_LINES - 1)
