...
export function createSignerFromKeystore(
  ...
  async function keyIdFor(address: string): Promise<string> {
    ...
    if (!entry) {
      throw new Error(
        `No key in the keystore daemon for address ${address} (is it generated? run: keystore list)`,
      )
    }
    return entry.keyId
  }
...
export async function createKeystoreSigner(
  ...
  const secrets = (keystore as unknown as { secrets?: import('./dispenser.js').SecretsLike })
    .secrets
