 ...
 *   curl -X POST :8789/query/lookup_asset -d '{"assetId":31566704}'
 */
...
const rest = createQueryHandler({ network: 'testnet', mode: 'compose', tools: defaultTools, plugins: defaultPlugins() })
...
Bun.serve({
  ...
  async fetch(request) {
    ...
    if (pathname === '/query' && request.method === 'GET') return Response.json({ tools: rest.catalogue() })
    if (pathname === '/buy' && paywall) return request.method === 'POST' ? paywall.buy(request) : Response.json(await paywall.status(request))
    const name = pathname.match(/^\/query\/([a-z0-9_]+)$/)?.[1]
