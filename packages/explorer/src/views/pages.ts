 ...
 */
export function nextPageArgs(result: StructuredResult | undefined): Record<string, unknown> | undefined {
...
export function mergePages(
  ...
  if (first.state !== 'success' || next.state !== 'success') throw new Error('Cannot merge a failed page')
  ...
  return record({ ...identity, ...(first.input === undefined ? {} : { input: first.input }) }, first.toolName, {
    ...rest,
    [key]: rows,
