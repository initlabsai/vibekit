...
export const ecosystemTools: ToolDefinition[] = [
  {
    ...
    parameters: z.object({
      category: categoryEnum.optional().describe('Filter by category. Use alone only when the user wants to browse an entire category.'),
      query: z.string().optional().describe('Search term to filter by name, description, or features. Always provide when the user asks about a specific topic, not just a category.'),
    }),
    handler: async ({ args }) => {
