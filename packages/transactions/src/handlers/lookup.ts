...
import { stripFinalToken } from '@vibekit/core'
...
export async function lookupTransaction(
  ...
  const response = await algorand.client.indexer.lookupTransactionByID(args.txid).do()
...
export async function lookupTransactionGroup(
  ...
  const response = await algorand.client.indexer.searchForTransactions().groupid(args.groupId).limit(100).do()
