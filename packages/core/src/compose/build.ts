...
function requireAddress(value: string | undefined, what: string, index: number): string {
  if (!value || !algosdk.isValidAddress(value)) {
    throw new ToolError('INVALID_ADDRESS', `Transaction ${index}: ${what} must be a valid address (got: ${value})`)
  }
  return value
}
...
export function resolveAbiMethod(
  ...
  if (spec.appSpec && spec.method) {
    ...
    if (!methodDef) {
      throw new ToolError('METHOD_NOT_FOUND', `Transaction ${index}: method "${spec.method}" not found in app spec`)
    }
...
export async function buildGroup(
  ...
  for (let i = 0; i < specs.length; i++) {
    ...
    switch (spec.type) {
      ...
      default:
        throw new ToolError('UNKNOWN_TXN_TYPE', `Transaction ${i}: unknown type "${(spec as TxnSpec).type}"`)
    }
  }
