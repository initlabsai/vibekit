...
export async function commandNew(args: string[]): Promise<void> {
  ...
  const dirInput =
    ...
    (await text({
      ...
      validate: (value) =>
        (value ?? "").trim() === "" ? "Directory is required" : undefined,
    }));
  ...
  if (parsed.noInit) {
    p.log.info(
      "Skipped agent setup (--no-init) — run `vibekit init` inside the project any time.",
    );
  } else if (
    headless ||
    (await confirm("Set up AI coding agents in this project?", true))
  ) {
