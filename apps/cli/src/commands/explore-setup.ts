...
function zeroSignalInstallSteps(): string {
  ...
  switch (process.platform) {
    ...
    default:
      return [
        `download zs-proxy_<version>_linux_${arch}.tar.gz from https://github.com/TxnLab/zs-proxy/releases`,
        'verify it against checksums.txt, extract, and put zs-proxy on your PATH',
      ].join('\n  ')
  }
}
...
async function pickModel(models: string[], providerLabel: string): Promise<string> {
  if (models.length === 0) {
    return text({ message: `No models listed — enter a ${providerLabel} model id` })
  }
...
Docs: https://txnlab.gitbook.io/zerosignal`)
...
async function ollamaFlow(): Promise<StoredAgentConfig> {
  ...
  try {
    ...
    p.log.warn('Ollama is not answering on :11434 — start it (or pick a model id to use later).')
  }
...
async function keyedFlow(
  ...
  if (process.env[envVar]) {
    p.log.success(`${envVar} found in your environment — it stays there; nothing is written to disk.`)
  } else {
    p.log.warn(`${envVar} is not set. The Explorer reads it from your environment at launch; export it before running vibekit explore.`)
  }
...
export async function commandExploreSetup(args: string[]): Promise<void> {
  ...
  const provider = await select({
    ...
    options: [
      { value: 'zerosignal', label: 'ZeroSignal', hint: 'decentralized, pay-per-message USDC, no API key' },
      { value: 'ollama', label: 'Ollama', hint: 'local models, no API key' },
      { value: 'anthropic', label: 'Anthropic', hint: 'needs ANTHROPIC_API_KEY in env' },
      { value: 'openai', label: 'OpenAI', hint: 'needs OPENAI_API_KEY in env' },
      { value: 'openai-compatible', label: 'OpenAI-compatible endpoint', hint: 'custom base URL' },
    ],
  })
  ...
  switch (provider) {
    ...
    case 'ollama':
      ...
      break
    case 'anthropic':
      config = await keyedFlow('anthropic', 'ANTHROPIC_API_KEY', 'claude-sonnet-5')
      break
    case 'openai':
  ...
  p.outro(`Saved to ${path} — run ${pc.cyan('vibekit explore')} to chat. Env vars (VIBEKIT_AGENT_*) still override.`)
}
