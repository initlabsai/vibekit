...
export function explorerSystemPrompt(
  ...
  return [
    ...
    "Every tool result becomes a card the user sees. Your reply after tools is ONE short sentence saying what you did (which lookups, how many results). NEVER list, enumerate, restate, or reformat the data — no markdown, no bullets, no tables, no ids, no amounts, no per-transaction breakdown. The cards are the answer. The only exception: the user explicitly asks you to analyze, explain, compare, or summarize the data, and then stay brief.",
