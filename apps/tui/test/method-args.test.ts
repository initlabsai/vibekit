...
describe('method line', () => {
  test('tokens split on space/comma but keep quotes and brackets whole', () => {
    expect(splitTokens(`"hi there" 1, [1,2] {"a":"b c"} x=y`)).toEqual(['"hi there"', '1', '[1,2]', '{"a":"b c"}', 'x=y'])
  })
  ...
  test('positional, name=value, and JSON forms all land on ABI names with coerced values', () => {
    ...
    const pairs = parseMethodArgs(method, `arg6=[1] deposit={"type":"pay"} fast=false asa=1 who=${ADDR} amount=0 note=x`)
  ...
  test('type errors name the arg; missing and extra args are caught', () => {
    ...
    expect(parseMethodArgs(hello, '')).toMatchObject({ ok: false, error: 'hello needs 1 arg: name' })
    expect(parseMethodArgs(hello, 'a b')).toMatchObject({ ok: false, error: expect.stringContaining('too many') })
    expect(parseMethodArgs(method, 'x notanumber')).toMatchObject({ ok: false, error: expect.stringContaining('amount — uint64') })
    expect(parseMethodArgs(method, `x 1 ${ADDR}`)).toMatchObject({ ok: false, error: expect.stringContaining('missing: asa') })
    expect(parseMethodArgs(hello, 'nope=1')).toMatchObject({ ok: false, error: 'no arg named nope' })
