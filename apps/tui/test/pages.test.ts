...
import { buildAssetHoldingsRecord, createResultStore, type ResultStore } from '@initlabs/vibekit-explorer'
...
describe('loadNextPage', () => {
  test('re-runs the record\'s own call with its token, merges, and returns a view over the merged record', async () => {
    ...
    const second = buildAssetHoldingsRecord(identity(2), { address: SENDER, assets: [holding(2), holding(3)] })
    ...
    const data = merged?.state === 'success' ? (merged.data as { assets: unknown[]; nextToken?: string }) : undefined
