...
describe('agent route', () => {
  ...
  test('a turn streams narration, drafts a composed group, bridges a read, and returns the new history', async () => {
    ...
    // The house pays for two program pages a turn and declines a third; every other gated call —
    // the compose-only writes above all — passes, because the wallet is the real gate.
