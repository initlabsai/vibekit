  ...
  const runAgent = useCallback(
    async (sectionId: number, input: string) => {
      if (!status.enabled) {
        appendNote(
          sectionId,
          'No agent configured. Paste an id, or `pay 0.5 to <address>`.',
          'error',
        )
        return
      }
      ...
      if (live !== true) {
        appendNote(
          sectionId,
          `The agent needs a live network — ${networkRef.current} is unreachable.`,
          'error',
        )
        return
      }
      ...
      updateItem(sectionId, noteId, (item) =>
        item.kind === 'note' ? { ...item, pending: true } : item,
      )
      let pending = true
      let spoke = false
      const setNote = (text: string, isPending: boolean) =>
        updateItem(sectionId, noteId, (item) =>
          item.kind === 'note' ? { ...item, text, pending: isPending } : item,
        )
      ...
      try {
        ...
        if (response.status === 402 || response.status === 429) {
          ...
          const payload = (await response.json().catch(() => ({}))) as {
            error?: string
            offer?: { price: string; turns: number }
          }
          const text =
            response.status === 402
              ? outOfTurnsLine(sectionId, payload.offer)
              : (payload.error ?? 'the house is busy. give me a minute.')
          updateItem(sectionId, noteId, (item) =>
            item.kind === 'note' ? { ...item, text, pending: false, mood: 'squint' } : item,
          )
          pending = false
          spoke = true
          return
        ...
        appendNote(
          sectionId,
          `Agent failed — ${shorten(error instanceof Error ? error.message : String(error), 160)}`,
          'error',
        )
      } finally {
        setBusy(false)
      ...
      try {
        ...
        if (view !== undefined) {
          ...
          const text = JSON.stringify(
            record.state === 'success' ? record.data : record.error,
            null,
            2,
          )
        ...
        appendNote(
          sectionId,
          `Dropped a malformed result from ${event.toolName} — ${shorten(error instanceof Error ? error.message : String(error), 100)}`,
          'error',
        )
      }
    },
