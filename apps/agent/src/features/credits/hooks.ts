...
export function useCredits({
  ...
  /** Pays for one pack through the connected wallet; resolves to the refreshed balance. */
  const buy = useCallback(async (): Promise<CreditsState> => {
    ...
    const [{ wrapFetchWithPayment, x402Client }, { ExactAvmScheme }] = await Promise.all([import('@x402/fetch'), import('@x402/avm/exact/client')])
