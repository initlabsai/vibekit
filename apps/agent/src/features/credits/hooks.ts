...
export function useCredits({
  ...
  const buy = useCallback(async (turns?: number): Promise<{ state: CreditsState; txid?: string }> => {
    ...
    if (!response.ok) {
      ...
      try {
        reason = required ? (JSON.parse(atob(required)) as { error?: string }).error : undefined
      } catch {
        // an unreadable header still leaves the status
      }
      const body = (await response.json().catch(() => ({}))) as { error?: string }
      throw new Error(body.error ?? (reason && reason !== 'Payment required' ? reason : `payment failed (${response.status})`))
    }
    try {
